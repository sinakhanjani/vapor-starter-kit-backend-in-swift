...
class TGJU {
    ...
    private func updateCurrencyBuilder(request: Request, dict: [String:[String]]) {
        self.parseHTML(request: request,dict: dict, completion: { currencyBuilders in
    ...
    private func parseHTML(request: Request,dict: [String:[String]], completion: @escaping (_ results: [CurrencyBuilder]?) -> Void) {
        do {
            let tgjuURL = self.refType.tgjuURL()
            _ = try? request.client().get(tgjuURL).map { (res) in
